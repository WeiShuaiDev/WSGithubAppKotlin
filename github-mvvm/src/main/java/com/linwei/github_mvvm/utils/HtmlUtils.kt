...
import com.linwei.cams.ext.colorIdToString
...
object HtmlUtils {
    ...
    fun generateHtml(
        ...
        val backgroundColor: String = backgroundColorId.colorIdToString()
        ...
        try {
            ...
            for (m in tags) {
                ...
                if (capture?.indexOf("<code>") != null && capture.indexOf("http://") < 0 && capture.indexOf(
                        "https://"
                    ) < 0 && capture.indexOf("#") != 0
                ) {
    ...
    fun parseDiffSource(diffSource: String?, wrap: Boolean): String {
        ...
        for (i in 0 until lines.size) {
            ...
            lineNumberStr =
                getDiffLineNumber(
                    if (curRemoveNumber == -1) "" else (curRemoveNumber.toString() + ""),
                    if (curAddNumber == -1) "" else (curAddNumber.toString() + "")
                )
            source =
    ...
    fun getDiffLineNumber(removeNumber: String, addNumber: String): String {
        val minLength = 4
        return getBlank(minLength - removeNumber.length) + removeNumber + getBlank(1) + getBlank(
            minLength - addNumber.length
        ) + addNumber;
    }
